#include <iostream>
#include <fstream>
#include <sstream>
#include <unordered_map>
#include <vector>
#include <string>
#include <regex>
#include <algorithm>
#include <chrono>

using namespace std;

string format_number(long number) {
    stringstream ss;
    ss.imbue(locale(""));
    ss << fixed << number;
    return ss.str();
}

void process_population_data(const vector<string>& all_lines, unordered_map<string, long>& data) {
    const string HEADERS[] = {
        "1960", "1961", "1962", "1963", "1964", "1965", "1966", "1967", "1968", "1969",
        "1970", "1971", "1972", "1973", "1974", "1975", "1976", "1977", "1978", "1979",
        "1980", "1981", "1982", "1983", "1984", "1985", "1986", "1987", "1988", "1989",
        "1990", "1991", "1992", "1993", "1994", "1995", "1996", "1997", "1998", "1999",
        "2000", "2001", "2002", "2003", "2004", "2005", "2006", "2007", "2008", "2009",
        "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019",
        "2020", "2021", "2022", "2023"
    };

    for (const string& line : all_lines) {
        regex re(R"(",")");
        sregex_token_iterator it(line.begin(), line.end(), re, -1);
        sregex_token_iterator end;

        vector<string> tokens(it, end);

        for (auto& token : tokens) {
            token.erase(remove(token.begin(), token.end(), '"'), token.end());
        }

        if (tokens[0] == "Not classified") {
            continue;
        }

        size_t i = 4;
        while (i < tokens.size()) {
            if (!tokens[i].empty()) {
                long population = stol(tokens[i]);
                data[HEADERS[i - 4]] += population;
            }
            ++i;
        }
    }
}

void read_population_data(const string& file_name, unordered_map<string, long>& data) {
    ifstream file(file_name);
    string line;

    for (int i = 0; i < 5 && getline(file, line); ++i);

    vector<string> all_lines;
    while (getline(file, line)) {
        all_lines.push_back(line);
    }

    file.close();
    process_population_data(all_lines, data);
}

// Main function
int main() {
    string filename = "../data/worldbank population/API_SP.POP.TOTL_DS2_en_csv_v2_3401680/API_SP.POP.TOTL_DS2_en_csv_v2_3401680.csv";
    
    auto start_time = chrono::high_resolution_clock::now();
    unordered_map<string, long> data;
    read_population_data(filename, data);

    vector<pair<string, long>> sorted_data(data.begin(), data.end());
    sort(sorted_data.begin(), sorted_data.end());

    cout << "Total population by year:" << endl;
    for (const auto& [year, population] : sorted_data) {
        cout << year << ": " << format_number(population) << endl;
    }

    auto end_time = chrono::high_resolution_clock::now();
    chrono::duration<double> execution_time = end_time - start_time;

    cout << "\nExecution time: " << execution_time.count() << " seconds" << endl;

    return 0;
}
